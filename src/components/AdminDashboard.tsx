...
import { Search, Filter, Download, Eye, Copy, BarChart3 } from 'lucide-react';
...
interface AdminDashboardProps {
  ...
  loading?: boolean;
  onRefresh?: () => void;
  onShowDetailedAnalytics?: (filterType: 'view' | 'download' | 'copy' | 'all') => void;
  onShowAssetAnalytics?: () => void;
}
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-brand-white p-6 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-primary mx-auto mb-4"></div>
          <p className="text-brand-gray font-calibre">Loading analytics...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <div className="flex items-center gap-6 mb-4">
            ...
            {onRefresh && (
              <button
                onClick={onRefresh}
                className="ml-auto px-4 py-2 bg-brand-primary text-white rounded-lg font-axiforma hover:bg-brand-dark transition-colors"
              >
                Refresh Data
              </button>
            )}
